...
public class MessageAdapter extends FirebaseRecyclerAdapter<MessagesModel, MessageAdapter.MessageViewHolder> {
    ...
    public MessageViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == SENDER_LAYOUT){
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.msg_sent_item, parent, false);
            return new MessageViewHolder(view);
        }
        else { // RECEIVER_LAYOUT
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.msg_receive_item, parent, false);
            return new MessageViewHolder(view);
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (getItem(position).getCustomer_id() != null
                && getItem(position).getCustomer_id().equals(my_id)){
